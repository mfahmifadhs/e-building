    ...
    <div class="row mb-2">
      ...
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
          ...
          <li class="breadcrumb-item active"><b>Data Pegawai Cleaning Service</b></li>
        </ol>
      </div>
    </div>
  </div>
      ...
      <div class="col-12">
        ...
        <div class="card">
          <div class="card-header" style="background-color:#11B7AE;color: white;">
            <h3 class="card-title-data"><b>Data Pegawai Cleaning Service</b></h3>
            <!-- <form action="{{ url('admin-master/delete_all_cleaning_service') }}" method="POST">
              @csrf
              <button class="btn btn-danger add-data" onclick="return confirm('Yakin ingin menghapus semua data?')">
                <i class="fas fa-trash-alt"></i>
              </button>
            </form> -->
            <a href="#add-import-pegawai" data-toggle="modal" data-target="#add-import-pegawai" class="btn btn-success add-import-pegawai">
              <i class="fas fa-file-upload"></i>
            </a>
            <a href="#add-pegawai" data-toggle="modal" data-target="#add-pegawai" class="btn btn-primary add-data">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td class="text-center">                        
                    <!-- <form action="{{ url('admin-master/delete_cleaning_service',$row->id_employee) }}" method="POST">
                      @csrf -->
                      <a class="btn btn-warning btn-md" href="{{ url('admin-master/edit_cleaning_service',$row->id_employee) }}">
...
<div class="modal fade" id="add-import-pegawai" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header" style="background-color:#11B7AE;color: white;">
        <h5 class="modal-title" id="exampleModalLabel"><b>{{ Auth::user()->name }} - Upload Data Cleaning Services</b></h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <form action="{{ url('admin-master/upload_cleaning_service/') }}" method="POST" enctype="multipart/form-data">
...
<!-- Modal Add Pegawai -->
<div class="modal fade" id="add-pegawai" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header" style="background-color:#11B7AE;color: white;">
        <h5 class="modal-title" id="exampleModalLabel"><b>{{ Auth::user()->name }} - Tambah Data Cleaning Services</b></h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <form action="{{ url('admin-master/add_cleaning_service/') }}" method="POST">
        <input type="hidden" name="user_id" value="{{ Auth::user()->id }}">
        <input type="hidden" name="status_id" value="1">
        <input type="hidden" name="emp_category" value="Cleaning Service">
        @csrf
        <div class="modal-body">
          <div class="row">             
            <div class="col-md-6">
            ...
            <div class="col-md-6">
              <div class="form-group">
                ...
                <select class="form-control" name="user_id" required>
                  ...
                  <option value="2">PT. SENDIKA PERKASA MEGAUTAMA</option>
                </select>
              </div>
            </div>                 
            <div class="col-md-6">
